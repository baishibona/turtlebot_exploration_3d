    ...
    while (ros::ok())
    {
        ...
        while (!arrived) {
            ...
            if(arrived)
            {
                ...
                explo_log_file << "Bay Step ," << robot_step_counter << ", Current Entropy ," << countFreeVolume(cur_tree) << ", time, " << ros::Time::now().toSec() - start_time_secs << endl;
